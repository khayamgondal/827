...
decorator // Used in: decorators
	: AT dotted_name LPAR opt_arglist RPAR NEWLINE// {std::cout<<"AT dotted_name LPAR opt_arglist RPAR NEWLINE\n"; }
	| AT dotted_name NEWLINE// {std::cout<<"AT dotted_name NEWLINE\n"; }
	;
...
parameters // Used in: funcdef
	: LPAR varargslist RPAR// {std::cout<<"lrpar"; }
...
print_stmt // Used in: small_stmt
	: PRINT opt_test { $$ = new PrintBinaryNode($2, NULL); 
			Scope::scope.at(currentIndex)->stmts.push_back($$) ; 
			pool.add($$);					
...
star_COMMA_test // Used in: star_COMMA_test, opt_test, listmaker, testlist_comp, testlist, pick_for_test
	: star_COMMA_test COMMA test// {std::cout<<"star_COMMA_test COMMA test\n"; }
	| %empty
	;
...
exec_stmt // Used in: small_stmt
	: EXEC expr IN test opt_COMMA_test// {std::cout<<"EXEC expr IN test opt_COMMA_test";}
	| EXEC expr// {std::cout<<"EXEC expr";}
	;
...
if_stmt // Used in: compound_stmt
	: IF test COLON suite star_ELIF ELSE {
...
old_lambdef // Used in: old_test
	: LAMBDA varargslist COLON old_test// {std::cout<<"LAMBDA varargslist COLON old_test\n";}
...
comparison // Used in: not_test, comparison
	: expr {$$=$1;}
	| comparison comp_op expr {$$ = new CompBinaryNode($1, $3, $2); 
					pool.add($$);	
					Scope::scope.at(currentIndex)->stmts.push_back($$) ; 
...
arith_expr // Used in: shift_expr, arith_expr
	...
	| arith_expr pick_PLUS_MINUS term {  
		if ($2 == 1) { $$ = new AddBinaryNode($1, $3); Scope::scope.at(currentIndex)->stmts.push_back($$); 		if (currentIndex == 0) $$->eval(); 					pool.add($$);					
				}
		if ($2 ==2) {$$ = new SubBinaryNode($1, $3); Scope::scope.at(currentIndex)->stmts.push_back($$); 		if (currentIndex == 0) $$->eval();					pool.add($$);					
					}
...
power // Used in: factor
	: atom star_trailer DOUBLESTAR factor {$$= NULL; } // {std::cout<<"atom star_trailer DOUBLESTAR factor \n";} 
	| atom star_trailer  { 
		if ($2 == 1) { 
...
star_trailer // Used in: power, star_trailer
	...
	;
atom // Used in: power
	: LPAR opt_yield_test RPAR {$$= NULL; } // {std::cout<<"atom\n";}
...
lambdef // Used in: test
	...
	| LAMBDA COLON test// {std::cout<<"LAMBDA COLON test\n";}
	;
trailer // Used in: star_trailer
	: LPAR opt_arglist RPAR// {std::cout<<"LPAR opt_arglist RPAR\n";}
...
classdef // Used in: decorated, compound_stmt
	: CLASS NAME LPAR opt_testlist RPAR COLON suite// {std::cout<<"class";}
...
argument // Used in: star_argument_COMMA, star_COMMA_argument, pick_argument
	: test opt_comp_for { $$= NULL;  Scope::scope.at(currentIndex)->stmts.push_back(new ArgBinaryNode($1));}
	| test EQUAL test {$$ = new AsgBinaryNode($1, $3); $$->eval(); 	pool.add($$); /*Scope::scope.at(currentIndex)->stmts.push_back($$) ;*/ std::cout<<"EQUAL NODE ";}
	;
